...
unsigned predictorMode = 1;

void set_script_parameters(int mode, unsigned lps, unsigned lcb, unsigned lhts, unsigned gps, unsigned cps)
{
    if(mode == 1) { //2 bit local
        localPredictorSize = lps;
        localCounterBits = lcb;
    } else if(mode == 2) { //tournament
        localHistoryTableSize = lhts;
        globalPredictorSize = gps;
        choicePredictorSize = cps;
    } else if(mode == 3) { //gshare
        globalPredictorSize = gps;
    } else { //default mode, runs the 2 bit local with default parameters
        mode = 1;
    }
...
bool predict(Branch_Predictor *branch_predictor, Instruction *instr)
{
    ...
    if(predictorMode == 1) {  
        ...
        unsigned global_predictor_idx = branch_predictor->global_history ^ branch_address;
        global_predictor_idx = global_predictor_idx & branch_predictor->global_history_mask;
